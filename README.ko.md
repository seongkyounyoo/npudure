...
> 프로젝트도 코드도 측정 결과도 동일하다. Phase 1 구현 식별자(`npuforge-*`
> 크레이트, proto 패키지, 서비스 유닛, 실험 스크립트)는 호환성과 실험 출처를
> 지키기 위해 그대로 둔다. 421건의 측정이 그 이름으로 이뤄졌으니, 기록은
> 기록된 당시의 이름을 유지하는 것이 맞기 때문이다.

저비용 NPU 여러 대를 표준 이더넷으로 묶어 분산 AI 추론을 확장하는 오픈소스
**Edge NPU Cluster** 런타임이다. Rust로 썼고 평범한 gRPC만 쓴다 — 전용 전송
계층도, RDMA도, 커널 우회도 없다.

**Edge NPU Cluster**는 모델을 통째로 하나씩 지닌 저렴한 NPU 보드 여럿을 스케줄러
하나가 앞에서 요청당 한 대씩 배정하는 구조다. 초당 처리량을 올린다. 여러
보드를 더 큰 가속기 한 대로 합치는 것도, 요청 하나를 더 빨리 만드는 것도
아니다.
...
| | |
|---|---|
| **정체** | Edge NPU Cluster 런타임 — 스케줄러 하나가 독립적인 추론 요청을 저비용 NPU 보드 여럿에 평범한 이더넷으로 분배한다 |
...
| **1 → 3노드 처리량** | 112.9 → 338.4 inf/s — **3.00×** |
...
문서는 한 방향으로 썼고 반대 방향으로 읽는 것이 좋다. 그 차이가 이 프로젝트다 —
계획한 것과 만든 것이 다르고, 그 차이를 측정이 정했다.

```text
쓴 순서   PRD ------> TECHSPEC ----> experiments ----> ADR / RESULTS
          "이걸 해보자"  "이렇게 만들자"  "실제로 해보니?"   "틀렸다 / 맞다 / 바꿨다"

읽는 순서 README --> 실험 대장 --> RESULTS + ADR --> TECHSPEC --> PRD
          무엇인가   무엇을 묻고     왜 그렇게 됐나   어떻게    무엇을
                     무엇을 배제했나                    도는가   계획했나
```

마지막 둘은 순서가 아니라 위치가 값어치다. TECHSPEC 은 io_uring 배제 판정이 기록된
곳이고(§15) 코드와 함께 읽는 문서다. PRD 는 손대지 않은 계획 기준선이다 — 여러 항목이
나중에 측정으로 배제되거나 미뤄졌고, 그 변화가 보이도록 그대로 둔다.

| 문서 | 내용 |
|---|---|
| **[`docs/experiments/README.md`](docs/experiments/README.md)** | **실험 대장 — 여기서 시작한다.** 무엇을 묻고 무엇이 배제됐는지 한 장. 원본 데이터 대응표와 방법론 교훈 포함 |
| **[`docs/FAQ.md`](docs/FAQ.md)** | **FAQ — 자주 받는 질문 11개.** 각 답변에 수치·측정 조건·원본 링크를 붙였다 (영문) |
...
나란히 둔다. 값이 갈리면 영문을 따른다. PRD 와 내부 작업 노트를 포함해
저장소의 모든 문서에 영문 정본이 있다.

가설 수립, 반대 검토, 문서화에 AI 어시스턴트를 썼다. 보고된 측정값은
전부 실장비에서 나온 것이고, 실험 결정과 공개 책임은 메인테이너에게 있다.
