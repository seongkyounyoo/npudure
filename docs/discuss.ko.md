...
*[English](discuss.md) — 영문이 정본이다.*

이 문서는 실험 설계와 해석에 영향을 준 기술 논의를 기록한다.

모든 실장비 실험과 측정, 구현 변경, 최종 기술 결정은 메인테이너가 직접
수행하고 책임진다.

Claude 와 ChatGPT 는 논의·검토 도구로 썼다. 이름을 남긴 것은 어떤 해석을
누가 제안했고 누가 반박했는지를 보존하기 위해서일 뿐이다.

**실험 수행 및 메인테이너: Seongkyoun Yoo**
...
| # | 절 | 작성 시각 (KST) | 의견/검토 출처 | 요지 |
|---|---|---|---|---|
| 1 | NPU 점유율 판별 실험 | 08-10 (시각 불명) | Claude 검토 | 최초 측정과 해석 |
| 2 | ChatGPT 답변/의견 | 08-10 (시각 불명) | ChatGPT 검토 | 표현 완화와 재검증 요구 |
| 3 | Claude 재검토 | 08-10 (시각 불명) | Claude 검토 | 지적 수용 및 재측정 |
| 4 | core_mask 분배 실험 | **08-10 17:03** | Claude 검토 | 대조군 추가, `worker_count=8` 확정 |
| 5 | want_float 실험 | **08-10 17:15** | Claude 검토 | 출력 변환 제거, +5.4% |
| 6 | syscall 분해 | **08-10 17:26** | Claude 검토 | 병목 확정: 드라이버 ioctl 직렬화 |
| 7 | zero-copy 실험 | **08-10 17:44** | Claude 검토 | 가설 반증 |
| 8 | INT8 실측 | **08-11 16:45** | Claude 검토 | **1.85배**. 6·7절 결론을 정교화 |
| 9 | 공유 컨텍스트 실험 | **08-11 16:45** | Claude 검토 | "오류 0건"은 정답이 아니다 |
| 10 | 벤치 도구 설계 | **08-11 17:15** | Claude 검토 | 실수를 도구에 박아 넣기 |
| 11 | CPU governor 영향 | **08-12 10:16** | Claude 검토 | **+7%**. 기존 수치는 전부 `ondemand` 기준 — **현재 유효한 결론** |
...
# want_float=0 전환과 CPU throttling — Claude 해석/의견
